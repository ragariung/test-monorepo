...
| Date | Change | Notes |
|---|---|---|
| 2026-09-07 | Added `Backends/src/applications/dto/update-lead.dto.ts` and `dto/decline-lead.dto.ts` | New admin `DRAFT`-lead management endpoints (edit/convert/decline) on the existing `admin-applications.controller.ts` — see `API-LIST-V0.md`'s matching revision row. No new module/controller file; `SimulatorModule` now exports `SimulatorService` (previously only `SimulationEngine`) so `ApplicationsService` can re-run a real simulation when a lead is edited. |
