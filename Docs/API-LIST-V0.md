...
| Date | Change | Notes |
|---|---|---|
| 2026-09-07 | New admin endpoint: `GET /admin/users/permissions`; `OrganizationView` fully wired | Prompted by a screenshot of the (previously mock) Organization screen — `POST /admin/users` and `PATCH /admin/users/:id/manager` already existed on the backend but were never called by the frontend; both now are (real "Undang Karyawan Baru" form, real inline manager-reassignment select). The hierarchy tree tab was 100% hardcoded JSX with only 4 of the real 7 seeded staff and fabricated tier/limit labels — replaced with a tree built at render time from `staffList`'s real `managerId` relationships. The capability matrix tab was a hand-typed array missing the `AUDITOR` role entirely and several newer permissions — replaced with a live read of `role-permissions.ts` via the new endpoint. Also added `StaffUser.isActive` to `Frontends/src/types.ts` (backend already returned it; the frontend just hardcoded an "Aktif" badge and ignored the real field — see `DATA-STRUCTURE.md`'s matching revision row). Verified live: create user (201, appears in list), manager reassignment (200, reflected immediately), permission matrix endpoint returns the real 6-role map matching `role-permissions.ts` exactly. |
| 2026-09-07 | New admin endpoints: `PATCH /admin/applications/:id/lead`, `POST /admin/applications/:id/convert`, `POST /admin/applications/:id/decline-lead` | Prompted by a real gap noticed while looking at a live `DRAFT` lead in the admin UI: staff had **no** way to act on a lead after calling the prospect — `startReview`/`approve`/`reject` all require `SUBMITTED`/`UNDER_REVIEW` and 409 on `DRAFT`, and there was no edit endpoint at all. New `applications:manage_lead` permission (granted to `UNDERWRITER_MANAGER`, `SENIOR_UNDERWRITER`, `UNDERWRITER`, `TELE_CONSULTANT` — same set as `applications:note`, not `AUDITOR`) covers all three. Along the way, fixed a real pre-existing frontend bug found during this work: `Frontends/src/types.ts`'s `ApplicationStatus` type never included `'Draft'` even though it flows through at runtime (`adapters.ts` papered over it with an `as ApplicationStatus` cast) — meaning `ApplicationDetailView`'s Approve/Reject buttons were incorrectly showing (and would 409) for a `DRAFT` lead. Type widened to include `'Draft'`, cast removed, action bar made status-aware (Edit/Convert/Decline for `DRAFT`, the original buttons otherwise). Also fixed a real display bug on the same page: the "Snapshot Simulasi" card rendered `Rp 0`/`0 Tahun` for a lead with no simulation attached instead of saying so — now shows an explicit "no simulation yet" state, gated on a new `hasSimulation` field. Verified live: edit (product/applicant/re-run simulation), convert (blocked with 400 until a simulation exists, then `DRAFT → SUBMITTED`), decline (`DRAFT → REJECTED` with a reason), and edit correctly 409s once no longer `DRAFT`. |
...
| PATCH | `/admin/applications/:id/lead` | session + `applications:manage_lead` | Implemented | **Not in the original architecture doc** — added for staff to correct/complete a `DRAFT` lead (chatbot-captured, `Automation/`) after calling the prospect. `DRAFT` only (409 otherwise). Every field optional/partial: `productId`, `applicant.*`, and `simulation` (`{age, sumAssured, paymentTermYears, paymentFrequency}` — if given, re-runs the real `SimulationEngine` via `SimulatorService`, never hand-typed premiums). Re-validates "at least one of email/phone" if either is touched |
| POST | `/admin/applications/:id/convert` | session + `applications:manage_lead` | Implemented | **Not in the original architecture doc** — `DRAFT → SUBMITTED` once staff have confirmed a lead by phone. Requires a simulation already attached (400 otherwise — via the chatbot or `PATCH .../lead`); enters the normal review pipeline (`start-review`/`approve`/`reject`) from there |
| POST | `/admin/applications/:id/decline-lead` | session + `applications:manage_lead` | Implemented | **Not in the original architecture doc** — `DRAFT → REJECTED` for a lead that never became a formal application (prospect unreachable/not interested). `reason` required (400 if blank). Deliberately separate from `reject()` above (different status guard, different permission — declining a lead that was never submitted is lower-stakes than rejecting a real underwritten application) |
...
| POST | `/admin/users` | session + `users:manage` | Implemented | Wired: `OrganizationView`'s "Undang Karyawan Baru" opens a real form (name, email, role, department, manager, temp password) |
| PATCH | `/admin/users/:id/manager` | session + `organization:manage` | Implemented | Walks the proposed manager chain upward; rejects direct and indirect cycles with 400 — verified during build. Wired: an inline select per row in `OrganizationView`'s employee table |
| GET | `/admin/users/permissions` | session + `users:manage` | Implemented | **Not in the original architecture doc** — serves `role-permissions.ts`'s `ROLE_PERMISSIONS` map as-is, so `OrganizationView`'s capability matrix tab reads the real source of truth instead of a hand-maintained frontend mirror (the mirror had already drifted — missing the `AUDITOR` role entirely and several newer permissions like `applications:manage_lead`) |
